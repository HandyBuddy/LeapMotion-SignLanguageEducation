...
one_hand_data = ""
one_hand_connected = False

class OneHandListener(Leap.Listener):

    rPrevPalmX = rPrevPalmY = rPrevPalmZ = 0.0
...
def remove_listener(controller, listener):
    global one_hand_data
    controller.remove_listener(listener)
    f = open("result.txt", 'r')
    one_hand_data = f.readline()
    f.close()
            
def main():
    global one_hand_connected
    # Create a sample listener and controller
    listener = OneHandListener()
    ...
    while True:
        if one_hand_connected == False:
        	print "reconnecting"
        	controller.remove_listener(listener)
        	controller.add_listener(listener)
        	time.sleep(0.1)
        elif one_hand_connected:
        	break
